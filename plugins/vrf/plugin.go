...
var log = logger.GetLogger("plugins/vrf")
...
// StaticRecords holds a vrf+MAC -> IP address mapping
...
func LoadDHCPv4Records(filename string) (map[string]net.IP, error) {
	...
	for _, lineBytes := range bytes.Split(data, []byte{'\n'}) {
		...
		vss := tokens[0]
		hwaddr, err := net.ParseMAC(tokens[1])
		if err != nil {
		...
		records[vss+":"+hwaddr.String()] = ipaddr
	}
...
func Handler4(req, resp *dhcpv4.DHCPv4) (*dhcpv4.DHCPv4, bool) {
	recLock.RLock()
	defer recLock.RUnlock()
	log.Debugf("req_options, %s", req.Options.String())
	rai := req.RelayAgentInfo()
	log.Debugf("RelayAgentInfo, %s", rai.String())

	vss := string(bytes.Trim(rai.Get(dhcpv4.VirtualSubnetSelectionSubOption), "\x00"))
	key := vss + ":" + req.ClientHWAddr.String()
	ipaddr, ok := StaticRecords[key]
	if !ok {
		log.Warningf("MAC address %s is unknown", req.ClientHWAddr.String())
		return resp, false
	}
	resp.YourIPAddr = ipaddr
	log.Debugf("found IP address %s for nik MAC %s", ipaddr, req.ClientHWAddr.String())
	return resp, true
}
